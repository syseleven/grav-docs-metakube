...
* You should use local storage nodes (flavors beginning with `l1.`) for the rook deployment.
* Rook supports only one shared file system. If you need mulitple shared file systems you need to set the env `ROOK_ALLOW_MULTIPLE_FILESYSTEMS=true`. This is an experimental feature! Further information can be found in the [ceph docs](http://docs.ceph.com/docs/master/cephfs/experimental-features/#multiple-filesystems-within-a-ceph-cluster) and the [rook docs](https://github.com/rook/rook/blob/master/Documentation/filesystem.md).
